...
interface TestGame2Props {
  onBackToMenu: () => void
}

type NumberSystem = 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 | 10

export function TestGame2({ onBackToMenu }: TestGame2Props) {
  ...
  const generateQuestion = useCallback(() => {
    ...
    // Случайно выбираем системы для каждого числа (от 2 до 10)
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm">
                      Правильный ответ: <span className="font-bold">
                        {correctAnswer === 'greater' ? 'Первое число больше (>)' : correctAnswer === 'less' ? 'Первое число меньше (<)' : 'Числа равны (=)'}
                      </span>
                    </p>
                  </div>
                )}
